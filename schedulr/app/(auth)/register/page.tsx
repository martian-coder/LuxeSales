...
export default function RegisterPage() {
  ...
  const handleRegister = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const result = await registerUser({ name, email, password, username });
      if (result?.error) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
      <div className="w-full max-w-md">
        {/* Logo */}
        <Link href="/" className="flex items-center gap-2 font-bold text-xl text-slate-900 justify-center mb-8">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8">
          ...
          {/* Value props */}
          <div className="bg-violet-50 border border-violet-100 rounded-xl p-4 mb-6 space-y-2">
            {["Unlimited bookings on free plan", "Public booking page in 60 seconds", "Google Calendar sync included"].map((v) => (
              <div key={v} className="flex items-center gap-2 text-sm text-violet-700">
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <span className="absolute inset-y-0 left-3 flex items-center text-xs text-slate-400">schedulr.app/</span>
                  <Input id="username" className="pl-24" placeholder="alex" value={username} onChange={(e) => setUsername(e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, ""))} required />
                </div>
              </div>
            </div>
            <div className="space-y-1.5">
            ...
            <Button type="submit" className="w-full bg-violet-600 hover:bg-violet-700 text-white" loading={isPending}>
              Create free account
            </Button>
          </form>
